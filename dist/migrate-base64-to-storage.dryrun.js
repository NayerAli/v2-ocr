...
function dryRunMigrateBase64ImageToStorage() {
    return __awaiter(this, void 0, void 0, function () {
        var _a, results, error, testResult, base64, buffer, path;
        return __generator(this, function (_b) {
            switch (_b.label) {
                ...
                case 1:
                    ...
                    testResult = results.find(function (r) { return !r.storage_path && r.image_url && r.image_url.startsWith('data:image/'); });
                    if (!testResult) {
                        console.log('No eligible test row found (no base64 image_url without storage_path).');
                        process.exit(0);
                    }
                    try {
                        base64 = testResult.image_url.split(',')[1];
                        buffer = Buffer.from(base64, 'base64');
                        path = "".concat(testResult.user_id, "/").concat(testResult.document_id, "/migrated_").concat(testResult.page_number || 1, ".jpg");
                        console.log('--- DRY RUN ---');
                        console.log("Would upload buffer of size: ".concat(buffer.length, " bytes to storage path: ").concat(path));
