...
// These imports are not used in this component
// import { getUser, getSession } from '@/lib/auth'
import { supabase } from '@/lib/supabase-client'

export default function AuthTestPage() {
  const { user, isLoading } = useAuth()
  // Define more specific types for the session and user objects
  type AuthUser = {
    id: string;
    email: string;
    [key: string]: unknown;
  ...
  const [serverSession, setServerSession] = useState<AuthSession | null>(null)
  ...
  const checkServerAuth = async () => {
    ...
    try {
      ...
      // Convert the server response to our typed objects
      if (data.user) {
  ...
  const checkClientAuth = async () => {
    ...
    try {
      ...
      // Convert the Supabase session to our ClientSession type
      if (data.session) {
  ...
  const memoizedCheckServerAuth = useCallback(checkServerAuth, [])
  const memoizedCheckClientAuth = useCallback(checkClientAuth, [])

  useEffect(() => {
    memoizedCheckServerAuth()
    memoizedCheckClientAuth()
  }, [memoizedCheckServerAuth, memoizedCheckClientAuth])
