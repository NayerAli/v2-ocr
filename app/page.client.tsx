...
import { getQueue, useCurrentUser } from '@/lib/database/client'
...
import { ConfigurationAlert } from "./components/api-key-missing-alert"
...
interface DashboardPageProps {
  configStatus: 'api-key-missing' | 'configuration-required' | 'configured';
}

export default function DashboardPage({ configStatus: serverConfigStatus }: DashboardPageProps) {
  ...
  // Override server configStatus if we have a valid API key client-side
  const [clientConfigStatus, setClientConfigStatus] = useState(serverConfigStatus)
  
  // Check if API key is valid on the client side
  useEffect(() => {
    if (user && !isAuthLoading && serverConfigStatus === 'api-key-missing') {
      const hasApiKey = settings.ocr.apiKey && settings.ocr.apiKey.length > 0
      const isUsingSystemKey = settings.ocr.useSystemKey === true
      
      console.log('[DEBUG] Client-side API key check:', { 
        hasApiKey, 
        isUsingSystemKey, 
        serverStatus: serverConfigStatus 
      })
      
      // If we have an API key or using system key, override the server status
      if (hasApiKey || isUsingSystemKey) {
        setClientConfigStatus('configured')
      }
    }
  }, [user, isAuthLoading, serverConfigStatus, settings.ocr])
  ...
  useEffect(() => {
    ...
    const loadQueue = async (isInitialLoad = false) => {
      ...
      try {
        ...
        let queue: ProcessingStatus[] = []
        if (userCurrent?.id) {
          queue = await getQueue(userCurrent.id)
        }
        if (isInitialLoad && process.env.NODE_ENV === 'development') {
        ...
        setStats({
          ...
          totalStorage: 0 // No dbStats in client, set to 0 or compute from queue if needed
        })
      } catch (error) {
  ...
  }, [isInitialized, user, isAuthLoading, userCurrent])
  ...
  return (
    <div className="min-h-screen">
      <div className="space-y-8">
        ...
        {/* Configuration alert - now using clientConfigStatus */}
        {clientConfigStatus !== 'configured' && user && !isAuthLoading && (
          <ConfigurationAlert type={clientConfigStatus} />
        )}
        ...
        <div className="relative">
          {!user && !isAuthLoading ? (
            ...
            <>
              {/* No alerts here - all alerts are shown at the top of the page */}
              <Card className={cn(
                ...
                disabled={clientConfigStatus !== 'configured' || !isInitialized}
