import type { OCRResult, OCRSettings, ProcessingStatus } from "@/types";
...
export class MistralOCRProvider implements OCRProvider {
  ...
  private readonly USE_MISTRAL_PDF_PROCESSING = true; // Changed to true to enable Mistral PDF processing
    ...
    while (retries <= this.maxRetries) {
      try {
        ...
        if (data && data.text) {
          ...
          responsePages = data.pages.length;
