  ...
  return {
    ...
    addToQueue: async (files: File[]): Promise<string[]> => {
      ...
      for (const file of files) {
        try {
          ...
          await db.syncWithServer(status);
    ...
    cancelProcessing: async (id: string) => {
      try {
        ...
        if (success) {
          ...
          const currentStatus = await db.getStatus(id);
          ...
          if (currentStatus) {
            ...
            await db.syncWithServer(updatedStatus);
          }
        }
    ...
    getStatus: async (id: string): Promise<ProcessingStatus | undefined> => {
      try {
        ...
        await db.syncWithServer(serverStatus);
