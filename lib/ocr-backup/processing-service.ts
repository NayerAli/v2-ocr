...
async function processPageByPage(
  ...
  try {
    ...
    let pdf;
    try {
      pdf = await loadPDFFromBuffer(pdfBuffer);
    } catch (pdfError) {
      ...
      return [{
        ...
        processingTime: 0,
        pageNumber: 1,
        error: pdfError instanceof Error ? pdfError.message : 'Error loading PDF',
      }];
    }
    ...
    for (let i = 1; i <= numPages; i++) {
      ...
      try {
        // Process the page
        const result = await processPage(pdf, i, documentId, ocrProvider, signal);
        results.push(result);
        ...
        // Add error result for this page but continue processing
        results.push({
          id: uuidv4(),
          documentId,
          text: '',
          ...
          processingTime: 0,
          pageNumber: i,
          error: pageError instanceof Error ? pageError.message : 'Error processing page',
        });
        ...
        if (ocrProvider instanceof MistralOCRProvider) {
          ...
          if (ocrProvider.canProcessPdfDirectly(fileSize)) {
            try {
              ...
              // Try page-by-page processing
              try {
                results = await processPageByPage(fileBuffer, status, ocrProvider, processing, signal, id);
              } catch (pageError) {
                console.error('Error in page-by-page processing:', pageError);
                
                // Last resort: treat PDF as a single image
                try {
                  console.log('Attempting to process PDF as a single document...');
                  const base64Data = await fileToBase64(fileBuffer, mimeType);
                  const result = await ocrProvider.processImage(base64Data, signal, mimeType);
                  result.documentId = id;
                  result.fileUrl = pdfUrl;
                  results = [result];
                } catch (finalError) {
                  console.error('All PDF processing methods failed:', finalError);
                  throw new Error('Failed to process PDF with any available method');
                }
              }
            }
          } else {
            ...
            try {
              ...
              // Fallback: treat PDF as a single image
              console.log('Attempting to process PDF as a single document...');
              const base64Data = await fileToBase64(fileBuffer, mimeType);
              const result = await ocrProvider.processImage(base64Data, signal, mimeType);
              result.documentId = id;
...
async function uploadImageToStorage(
  ...
  try {
    ...
    if (isSupabaseActive) {
      try {
        ...
        const filename = `${documentId}/${timestamp}-image.${extension}`;
  ...
  // Fall back to base64 for local development or if Supabase fails
  return `data:${mimeType};base64,${buffer.toString('base64')}`;
}
...
async function uploadFileToStorage(
  ...
  try {
    ...
    if (isSupabaseActive) {
      try {
        ...
        const timestamp = Date.now();
        const filename = `${documentId}/${timestamp}-${documentId}.${extension}`;
        ...
        if (error) {
          ...
          // Return empty string, processing will continue without a stored file URL
          return '';
        }
  ...
  // For local development without Supabase or if Supabase fails, return empty string
  return '';
}
